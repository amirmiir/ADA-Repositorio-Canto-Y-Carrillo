...
*/
constexpr size_t get_cell(size_t row, std::size_t col) noexcept {
    return (row / 3) * 3 + col / 3;
}

constexpr size_t get_next_row(size_t row, std::size_t col) noexcept {
    return row + (col + 1) / 9;
}
...
class Solution {
   public:
    void solveSudoku(std::vector<std::vector<char>> &board) const noexcept {
        /* Note: input is guaranteed to be a valid board using only '1'-'9' and '.' with a
           unique solution. Solution should modify board, not return a new one. */
   ...
   private:
    static constexpr std::pair<std::size_t, std::size_t> next_empty_position(
    ...
    static bool solve(std::vector<std::vector<char>> &board, std::size_t const row_start,
                      std::size_t const col_start, std::array<std::bitset<9>, 9> &row_contains,
...
int main() {
    std::array<char, 81> const flat_board = {
        ...
        '.', '.', '6', '.', '6', '.', '.', '.', '.', '2', '8', '.', '.', '.', '.', '4', '1',
        '9', '.', '.', '5', '.', '.', '.', '.', '8', '.', '.', '7', '9'};
    std::array<char, 81> const flat_expected = {
